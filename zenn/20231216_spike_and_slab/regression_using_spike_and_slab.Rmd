...
```{r}
bayes_reg_fit %>% 
  spread_draws(beta[index], sigma2) %>%
  summarise_draws()
```
...
```{r}
mcmc_intervals(weak_ss_reg_fit$draws(c("beta", "sigma2")), prob_outer = 0.95)
```
```{r}
mcmc_areas(weak_ss_reg_fit$draws(c("beta", "sigma2")), prob_outer = 0.95) +
  geom_point(data=data.frame(beta=true_beta, index=c(7,6,5,4,3,2)), aes(x=beta, y=index), color="red", shape=18, size=4)+
  geom_point(x=1,y=1, color="red", shape=18, size=4)
```
