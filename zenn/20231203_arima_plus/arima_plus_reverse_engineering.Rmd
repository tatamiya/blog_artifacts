...
stl_results <- stl(data_ts, s.window="per")
...
p_all <- ggplot(data=data_with_bqml) +
  geom_line(aes(x=day, y=y, color="actual", linetype="actual"))+
  geom_line(aes(x=day, y=pred_bqml, color="pred(BQML)", linetype="pred(BQML)"))+
  scale_color_manual(name="", values = c("pred(BQML)"="blue", "actual"="black")) +
  scale_linetype_manual(name="", values = c("pred(BQML)"="solid", "actual"="dashed")) +
  labs(x="day", y="y")
